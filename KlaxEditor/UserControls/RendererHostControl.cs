...
using KlaxRenderer.Graphics.RenderSurfaces;
using System;
...
{
	public class CRendererHostControl : HwndHost
	{
		public CRendererHostControl()
		{
			Loaded += OnLoaded;
		}
		...
		public IRenderSurface GetRenderSurface()
		{
			...
			{
				upperLeft = PointToScreen(new Point(0, 0));
			}
			return new HWNDHostSurface(Hwnd, (int)ActualWidth, (int)ActualHeight, (int)upperLeft.X, (int)upperLeft.Y);
		}
		...
		public void SetWorld(CWorld world)
		{
			...
			Point upperLeft = new Point();
			upperLeft = PointToScreen(new Point(0, 0));
			CEngine.Instance.Dispatch(EEngineUpdatePriority.BeginFrame, () => { World.ViewManager.ResizeView((float)ActualWidth, (float)ActualHeight, (float)upperLeft.X, (float)upperLeft.Y); });
		}
		...
		private void OnLoaded(object sender, RoutedEventArgs routedEventArgs)
		{
			...
			{
				parentWindow.LocationChanged += (obj, args) =>
				{
					OnSizeChanged((int)ActualWidth, (int)ActualHeight);
				};
			}
		...
		{
			return Input.ProcessCharacterInput((char)msg.wParam);
		}
		...
		protected override void OnRenderSizeChanged(SizeChangedInfo sizeInfo)
		{
			...
			if (HwndInitialized)
				OnSizeChanged((int)sizeInfo.NewSize.Width, (int)sizeInfo.NewSize.Height);
		}
		...
		private const string WindowClass = "HWND_WRAPPER";
	}
