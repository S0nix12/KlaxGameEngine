...
namespace KlaxRenderer.Graphics.ResourceManagement
{
	class CShaderRegistry : IDisposable
	{
		internal CShaderRegistry(Device device)
		{
			...
			m_shaderClassMap.Add(new SHashedName("textureShader"), typeof(CTextureSampler));
			...
			Parallel.Invoke(() => colorShader.Init(device), () => simpleLitShader.Init(device), () => textureShader.Init(device), () => uiShader.Init(device));
